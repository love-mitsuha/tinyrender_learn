...
void rasterize(Vec3f screen_coords[], Vec2f texture_coords[], Vec3f normal_coords[], Vec3f light_dir, TGAImage& image, TGAImage texture, float* zbuffer)
{
	//bbox必须使用int
	...
	for (P.x = xmin; P.x <= xmax; P.x++)
	{
		for (P.y = ymin; P.y <= ymax; P.y++)
		{
			...
			float alpha = P_normal * light_dir;
			if (alpha>0)
			{
